...
byte rowPins[ROWS] = {A11, A10, A9, A8}; // Analog pins A11 to A8 as row pins
byte colPins[COLS] = {A15, A14, A13, A12}; // Analog pins A15 to A12 as column pins
...
const char* accessPassword = "1234"; // Replace with your desired password
...
void displayHomePage() {
  ...
  // Display door lock status
  textYCenter += 30;
  tft.setCursor(textXCenter, textYCenter);
  if (isDoorLocked) {
    tft.setTextColor(0xFF0000); // Red text color
    tft.println("Door: Locked");
  } else {
    tft.setTextColor(0x00FF00); // Green text color
      ...
      if (!isAccessGranted) {
        ...
        while (passwordIndex < 4) {
          ...
          if (key) {
            ...
            enteredPassword[passwordIndex + 1] = '\0'; // Null-terminate the string
            passwordIndex++;
            tft.setCursor(20, 60);
            tft.println("****"); // Display asterisks for each digit entered
          }
        }

        if (strcmp(enteredPassword, accessPassword) == 0) {
          displayMessage("Access Granted");
          isAccessGranted = true;
          isDoorLocked = false; // Unlock the door
        } else {
        ...
        // Access is already granted, you can add additional logic or messages here
        displayMessage("Access already granted.");
        delay(1000);
        displayHomePage();
      }
      break;
...
void handleOptionsMenu() {
  // No need to check for key here; it's handled in the loop()
  switch (keypad.getKey()) {
    ...
    case '3':
      ...
      // For example, call a function to handle password reset
      // resetPasswordFunction();
      displayMessage("Password Reset Complete"); // Display a message to indicate the reset
      delay(2000);
      displayOptionsMenu();
      break;
